...
export default function CreateTestConfigurationPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    accessCode: '',
    maxAttemptsPerUser: 0, // 0 = no retries, 1 = one attempt, 2+ = multiple attempts
    retryAfterDays: 0,
...
4. Submission:
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.message || 'Failed to create test configuration');
      }

      console.log('Test configuration created:', result.data);
      showToast('Test configuration created successfully!', 'success');
      router.push('/admin/test-configurations');
      
    } catch (error) {
      console.error('Error creating test configuration:', error);
      showToast(error.message || 'Failed to create test configuration. Please try again.', 'error');
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center">
              ...
              <div>
                <h1 className="text-xl font-semibold text-gray-900">Create Test Configuration</h1>
                <p className="text-sm text-gray-600">Set up a new examination template</p>
              </div>
            </div>
